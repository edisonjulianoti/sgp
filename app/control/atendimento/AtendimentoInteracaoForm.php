...
class AtendimentoInteracaoForm extends TPage
{
    protected $form;
    private $formFields = [];
    ...
    public function __construct( $param )
    {
        ...
        $this->form->setFormTitle("Nova interação do atendente");
    ...
    public function onSave($param = null) 
    {
        try
        {
            ...
            $object->atendente_id = TSession::getValue('atendente_id');
            ...
            $templateEmail = TemplateEmailService::parseTemplate(TemplateEmail::NOVA_INTERACAO_ATENDENTE, $object);
            FilaEmailService::adicionaNaFila([$object->atendimento->cliente_usuario->system_user->email], $templateEmail->titulo, $templateEmail->conteudo, 'Nova Interação');
            ...
            SystemNotification::register( $object->atendimento->cliente_usuario->system_user_id, "Nova interação #{$object->id}", "Há um nova interação no atendimento #{$object->atendimento_id}", new TAction(['AtendimentoClienteFormView', 'onShow'], $notificationParam), 'Visualizar Atendimento', $icon);    

            TTransaction::close(); // close the transaction

            TelegramService::enviarMensagem("Há um nova interação no atendimento #{$object->atendimento_id}", $object->atendimento->cliente_usuario->system_user_id);
            ...
            TApplication::loadPage('AtendimentoInteracaoTimeLine', 'onShow', $pageParam);
