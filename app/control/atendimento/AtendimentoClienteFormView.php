...
class AtendimentoClienteFormView extends TPage
{
    protected $form; // form
    ...
    public function __construct( $param )
    {
        ...
        $btnNovaInteracao = new TActionLink("Nova Interação", new TAction(['AtendimentoInteracaoClienteForm', 'onShow'], ['atendimento_id'=> $atendimento->id]), '', '13px', '', 'fas:comment-medical #FF9800');
        $interacoes = new BPageContainer();

        $interacoes->setSize('100%');
        $interacoes->setAction(new TAction(['AtendimentoInteracaoClienteTimeLine', 'onShow'], ['atendimento_id' => $atendimento->id]));
        ...
        $row3 = $this->form->addFields([$label14,$text5],[$label16,$text7]);
        $row3->layout = [' col-sm-4',' col-sm-8'];

        $row4 = $this->form->addFields([$label18,$text10]);
        $row4->layout = [' col-sm-12'];

        $row5 = $this->form->addContent([new TFormSeparator("Interações do Atendimento", '#333', '18', '#eee')]);
        $row6 = $this->form->addFields([$btnNovaInteracao]);
        $row6->layout = [' col-sm-12'];

        $row7 = $this->form->addFields([$interacoes]);
        ...
        $style = new TStyle('right-panel > .container-part[page-name=AtendimentoClienteFormView]');
