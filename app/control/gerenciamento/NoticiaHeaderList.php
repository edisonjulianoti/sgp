...
class NoticiaHeaderList extends TPage
{
    private $form; // form
    ...
    public function __construct($param = null)
    {
        ...
        $id->exitOnEnter();
        $titulo->exitOnEnter();
        $descricao_resumida->exitOnEnter();
        ...
        $descricao_resumida->setExitAction(new TAction([$this, 'onSearch'], ['static'=>'1', 'target_container' => $param['target_container'] ?? null]));
        ...
        $column_descricao_resumida = new TDataGridColumn('descricao_resumida', "Descrição resumida", 'left');
        $column_data_noticia = new TDataGridColumn('data_noticia', "Data da notícia", 'left');
        ...
        $this->datagrid->addColumn($column_descricao_resumida);
        $this->datagrid->addColumn($column_data_noticia);
        $this->datagrid->addColumn($column_ativo_transformed);

        $action_onEdit = new TDataGridAction(array('NoticiaForm', 'onEdit'));
        ...
        $action_onDelete = new TDataGridAction(array('NoticiaHeaderList', 'onDelete'));
        ...
        $tr->add($td_titulo);
        $td_descricao_resumida = TElement::tag('td', $descricao_resumida);
        $tr->add($td_descricao_resumida);
        ...
        $this->datagrid_form->addField($descricao_resumida);
        ...
        $panel = new TPanelGroup("Listagem de notícias");
        ...
        $button_cadastrar->setAction(new TAction(['NoticiaForm', 'onShow']), "Cadastrar");
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Gerenciamento","Notícias"]));
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for Noticia
